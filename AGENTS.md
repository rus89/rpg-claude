...
This file mirrors `CLAUDE.md` so every AI agent working here follows the same rules.
Keep the two in sync — when one changes, change the other.
...
- Journal is the file JOURNAL.md at project root — read it at session start, append entries via your file editing tools.
...
- When spawning sub-agents, ALWAYS include this instruction in the prompt:
  "Read and follow all rules in AGENTS.md at the project root before starting any work."
...
- You MUST use your todo-tracking tool (TodoList, TodoWrite, or equivalent) to keep track of what you're doing
- You MUST NEVER discard tasks from your todo list without Milan's explicit approval
...
### Memory Hygiene (JOURNAL.md)

NEVER write the following to the journal as status — they rot and lie to future sessions:
...
The journal is for **durable facts**: architectural decisions, the *why* behind a choice, user preferences, reasoning that outlives a branch, lessons learned from incidents.

Before writing a journal entry, ask: "will this claim still be true in 3 weeks without anyone updating it?" If no, don't write it — re-derive from the authoritative source (git, filesystem, test runner) at session start.
