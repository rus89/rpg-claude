...
## 2026-04-30: Riverpod 2.x → 3.x migration (branch: chore/riverpod-3-migration)

### Migration outcome

`build_runner` is unblocked after bumping the riverpod stack to its 3.x/4.x line, which transitively pulls `analyzer 8.x` (with `visitDotShorthandInvocation` support). Resolved versions: `flutter_riverpod 3.1.0`, `riverpod_annotation 4.0.0`, `riverpod_generator 4.0.0+1`, `riverpod_lint 3.1.0`, `custom_lint 0.8.1` (had to bump from 0.7.x — it pinned `analyzer 7.x` and was blocking the upgrade), `analyzer 8.4.0`, `_fe_analyzer_shared 91.0.0`. The pub solver landed on the hybrid `flutter_riverpod 3.x` + `riverpod_annotation/generator 4.x` mix; runtime APIs are identical between 3.x and 4.x for our usage.

### Migration changes

- 13 mechanical `valueOrNull` → `.value` renames across 7 lib/ files + 1 integration_test file. In riverpod 3.x `.value` returns `null` during error states (was previously throwing); `valueOrNull` was removed. All 13 sites had `??`/`!= null` guards downstream so the semantic shift is absorbed safely.
- 4 redundant `flutter_riverpod` imports removed from `lib/providers/` files — `riverpod_annotation 4.x` re-exports the runtime types.
- 1 type fix in `test/screens/mapa_screen_test.dart`: dropped `List<Override>` annotation (the type became private in 3.x).
- 5 loading-screen tests: `tester.pump()` → `tester.pumpAndSettle()` for error-state assertions. Riverpod 3.x async resolution needs an extra microtask flush.
- All 7 `.g.dart` files regenerated by build_runner; both formerly hand-written placeholders (`preferences_provider.g.dart`, `review_prompter_provider.g.dart`) now have real generated hashes.

### Smoke test findings (pre-existing bugs — NOT migration regressions)

Manual smoke test on Android device (21051182G, API 33) surfaced two pre-existing issues confirmed via `git diff main...HEAD` to be unaffected by the migration:

1. **App version not displayed on "O aplikaciji" screen** — the `packageInfoProvider` is read but never rendered as a Text widget. It's only used to gate the feedback tile and embed the version in the feedback email body. Fix: add a `Text('Verzija: v${info.version}+${info.buildNumber}')` somewhere in the build tree.

2. **Line charts only show start/end date labels** (in `trendovi_screen.dart` line ~377 and `opstina_detail_screen.dart` line ~146). `dateToX(date)` returns `millisecondsSinceEpoch.toDouble()`, and `dateTicks.indexOf(value)` does an exact-equality match. fl_chart's auto-interval ticks almost never coincide with a millisecond timestamp, so only the chart's min/max bounds (which happen to equal the first/last `dateTicks` entries) render labels. Fix: set an explicit `interval` on `SideTitles` (1 year ≈ 31_557_600_000 ms) and map the auto-tick `value` back to the nearest date via interpolation, OR restructure the X axis to use indices 0..N with a separate label-mapping table.

Both issues exist on `main` too — they are out of scope for the riverpod migration.
